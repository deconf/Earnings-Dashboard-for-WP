...
class Google_Cache_Memcache extends Google_Cache_Abstract {
	...
	public function get( $key, $expiration = false ) {
		...
		if ( $ret === false ) {
			$this->client->getLogger()->debug( 'Memcache cache miss', array( 'key' => $key ) );
			return false;
		}
		if ( is_numeric( $expiration ) && ( time() - $ret['time'] > $expiration ) ) {
			$this->client->getLogger()->debug( 'Memcache cache miss (expired)', array( 'key' => $key, 'var' => $ret ) );
			$this->delete( $key );
			return false;
		}
		
		$this->client->getLogger()->debug( 'Memcache cache hit', array( 'key' => $key, 'var' => $ret ) );
	...
	public function set( $key, $value ) {
		...
		if ( $rc == false ) {
			$this->client->getLogger()->error( 'Memcache cache set failed', array( 'key' => $key, 'var' => $data ) );
			
			throw new Google_Cache_Exception( "Couldn't store data in cache" );
		}
		
		$this->client->getLogger()->debug( 'Memcache cache set', array( 'key' => $key, 'var' => $data ) );
	}
